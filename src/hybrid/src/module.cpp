...
namespace
{
    ...
    addon_value ExecSync(addon_env env, addon_callback_info info)
    {
        ...
        delete[] name; // Delete the dynamically allocated memory
        return msg;
    ...
    /*
     * The UXP Hybrid APIs have hard restrictions around thread usage
     *
     * For a simple async function example, reference MyAsyncEcho(), however note that this method
     * would lock up the App or Plugin depending on where the time-consuming task is placed in
     * the workflow.
     *
     * Running ExecSync() on the Main Thread would lock up the entire host app
     * Running ExecSync() on the Scripting Thread would lock up the entire UXP Plugin
     *
     * In order for Exec() to function asynchronously without locking up either the app or plugin
     * a series of thread spawns is required, and the sequence and rules around each sequence is crucial.
     *
     * Rules for Threading in UXP Hybrid
     * - The function must both start and complete in the Scripting thread (not the Main thread or spawned threads)
     * - Threads can only be spawned from the Main thread
     * - Tasks can only be scheduled on the Main thread from the Scripting thread
     * - Tasks can be scheduled on the Scripting thread from the Main thread or Spawned thread
     * - No UXP Values can be parsed, created, or updated in a Spawned thread, shared pointers must be used instead
     * - No UXP completion functions (e.g. uxp_addon_reject_deferred() or uxp_addon_resolve_deferred() ) can be run from non-scripting threads
     *
     * To satisfy these strict threading requirements, the function below follows this structure:
     * 1. [Scripting Thread] Start, Parse Command, Create Shared Pointers, Schedule task on Main thread
     * 2. [Main Thread] Move command from UXP value to string for spawned thread, spawn and detach from thread
     * 3. [Spawned Thread] Recieve command and run on appropriate OS (non-blocking), schedule task on Scripting thread
     * 4. [Scripting Thread] Recieve command result and either return as a resolve or reject
     *
     * Some additional documentation from Adobe can be found in the console: https://developer.adobe.com/console/62873/servicesandapis/ps (UXP Hybrid Plugin SDK)
     */
    addon_value Exec(addon_env env, addon_callback_info info)
    {
        try
        {
            ...
            // Create shared pointers to be used in each thread step
            auto valuePtr = std::make_shared<Value>(std::move(stdValue));
            ...
            auto mainThreadHandler = [valuePtr, outputPtr, isErrorPtr, scriptThreadHandler](Task &task)
            {
                try
                {
